...
denus = form['3132']  # Pendiente
denus = denus.loc[select_index]

atention = form["33"]
atention = atention.loc[select_index]

for x in atention.index:
    mun = refmap_muns.loc[x,'Municipio Mapa']
    resp_type = atention.loc[x,33]
    if mun in sonora_form.index:
        try:
            resp_type = resp_type.split(',')[0].capitalize()
        except AttributeError:
            continue
        sonora_form.loc[mun, "Atención"] = resp_type
...
if PLOT_MAPS:
    ...
    if selection_maps['Programa']:
        ...
        ax.set_axis_off()
    if selection_maps['Atencion']:
        atte, ax = plt.subplots(figsize = Mapper.FIGSIZE)
        sonora_form.plot('Atención', categorical = True, legend = True,
                        legend_kwds = {"title": "Atención"}, ax = ax)
        sonora.boundary.plot(color = "black", lw = 0.5, ax = ax)
        atte.tight_layout()
        ax.set_axis_off()
plt.show()
